...
double Npkt_ex[3] = {0,0,0}, Npkt_ob_ex[3] = {0,0,0};
...
void PhyTx(std::string context, Ptr<const Packet> p, double txPowerW) {
  if (p->GetSize() == 1064) {
    ...
    //NS_LOG_UNCOND(status_ex[0] << " " << status_ex[1] << " " << status_ex[2] << std::endl);

    if(ContextToNodeId(context) != 0){
      if( (ContextToNodeId(context) == 1 && status[2] == 0) || (ContextToNodeId(context) == 2 && status[1] == 0) ){
        Npkt_ex[1] ++;
        if(NumActive_ex(status_ex) == 0){
          Npkt_ob_ex[1] ++;
        }
        else if(NumActive_ex(status_ex) == 1){
          NiC2_ex[getOneActive_ex(status_ex)] ++; //N_{i -> i,j}, increment the NiC2[i], link j update now
        }
        status_ex[1] = true;
      }
      else if((ContextToNodeId(context) == 3 && status[4] == 0) || (ContextToNodeId(context) == 4 && status[3] == 0) ){
        Npkt_ex[2] ++;
        if(NumActive_ex(status_ex) == 0){
          Npkt_ob_ex[2] ++;
        }
    ...
    else if(ContextToNodeId(context) == 0){
      Npkt_ex[0] ++;
      if(NumActive_ex(status_ex) == 0){
        Npkt_ob_ex[0] ++;
      }
    ...
    else if(NumActive(status) == 1) { // N_{i -> i,j}, increment the NiC2[i]
...
void PhyTxEnd(int nodeID){
  ...
  if(nodeID != 0){
    if( (nodeID == 1 && status[2] == 0) || (nodeID == 2 && status[1] == 0) ){
      status_ex[1] = false;
    }
    else if( (nodeID == 3 && status[4] == 0) || (nodeID == 4 && status[3] == 0) ){
      status_ex[2] = false;
    }
  }
  else if(nodeID == 0){
...
void PhyRx(std::string context, Ptr<const Packet> p) {
  if (p->GetSize() == 1064) {
    ...
    if(FindSrc(srctable,p->GetUid())!= -1 && ContextToNodeId(context)-5 == FindSrc(srctable,p->GetUid())) {
      ...
      if(idx != 0){
        if( (idx == 1 && status[2] == 0) || (idx == 2 && status[1] == 0) ){
          status_ex[1] = false;
        }
        else if( (idx == 3 && status[4] == 0) || (idx == 4 && status[3] == 0) ){
          status_ex[2] = false;
        }
      }
      else if(idx == 0){
...
int main (int argc, char *argv[]){
  ...
  */
  ...
  r_ex[0] = static_cast<double>(NiC2_ex[0])/(Npkt_ob_ex[1] + Npkt_ob_ex[2]);
  r_ex[1] = static_cast<double>(NiC2_ex[1])/(Npkt_ob_ex[0] + Npkt_ob_ex[2]);
  r_ex[2] = static_cast<double>(NiC2_ex[2])/(Npkt_ob_ex[0] + Npkt_ob_ex[1]);
  ...
  if(outputmode == 1) {
    ...
    std::cout << "True pkt volume: " << Npkt[0]*1.0/(sim_time-1) << " " << Npkt[1]*1.0/(sim_time-1) << " " << Npkt[2]*1.0/(sim_time-1) << " " << Npkt[3]*1.0/(sim_time-1)
              << " " << Npkt[4]*1.0/(sim_time-1) << std::endl;
    std::cout << "Computed pkt volume: " << Npkt_ob[0]*(1+GetSoP(cg,r,0))*1.0/(sim_time-1) << " " << Npkt_ob[1]*(1+GetSoP(cg,r,1))*1.0/(sim_time-1) << " "
              << Npkt_ob[2]*(1+GetSoP(cg,r,2))*1.0/(sim_time-1) << " " << Npkt_ob[3]*(1+GetSoP(cg,r,3))*1.0/(sim_time-1) << " "
              << Npkt_ob[4]*(1+GetSoP(cg,r,4))*1.0/(sim_time-1) << "\n";
  ...
  //for(int i=0; i<=24; i++){ std::cout << cg[i] << ' ';}
    ...
    double Npkt_link1 = Npkt_ob_ex[0]*(1 + r_ex[1] + r_ex[2] + r_ex[1]*r_ex[2]);
    double Npkt_link2 = Npkt_ob_ex[1]*(1 + r_ex[0] + r_ex[2] + r_ex[0]*r_ex[2]);
    double Npkt_link3 = Npkt_ob_ex[2]*(1 + r_ex[0] + r_ex[1] + r_ex[0]*r_ex[1]);

    std::cout << sim_time << " " << Topology_Run << " " << mean << " "
              << Npkt_ex[0]*1.0/(sim_time - 1) << " " << Npkt_ex[1]*1.0/(sim_time - 1) << " " << Npkt_ex[2]*1.0/(sim_time - 1) << " "
              << Npkt_link1*1.0/(sim_time - 1) << " " << Npkt_link2*1.0/(sim_time - 1) << " " << Npkt_link3*1.0/(sim_time - 1) << " "
              << Npkt_ob_ex[0]*1.0/(sim_time - 1) << " " << Npkt_ob_ex[1]*1.0/(sim_time - 1) << " " << Npkt_ob_ex[2]*1.0/(sim_time - 1) << " "
              << NiC2_ex[0] << " " << NiC2_ex[1] << " " << NiC2_ex[2] << "\n";
  }
  return 0;
}
